...
mkdir -p output
output=./output/amqp-mqtt-unreliable-$(date -u "+%Y%m%d%H%M%S").log
echo test started > $output

./setup-base.sh

kubectl apply -f ../continuous-run-config/ -f ../100k-no-guarantee/ -f ../100k-messages-config/ -f ../cerk-printer-mqtt/ -f ../cerk-generator/ -f ../cerk-amqp-mqtt-only/
